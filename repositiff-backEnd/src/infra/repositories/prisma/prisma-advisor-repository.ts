...
`);
  ...
  async advisorExisting(idAdvisor: string): Promise<boolean> {
    try {
      const advisorExisting =
        (await this._prismaCli.advisor.count({
          where: {
            id: idAdvisor,
          },
        })) === 1;
  ...
  mapperToAdvisor(advisor: any): Advisor | Error {
    ...
    const advisorMappingToDomain = AdvisorFactory.createAdvisor(advisorProps, advisor.id);

...
export class PrismaAcademicWorkRepository implements IAcademicWorkRepository {
  ...
  async deleteAdvisorToAcademicWork(academicWorkId: string, advisorId: string): Promise<Error | void> {
    const fields = {};
    try {
      const entityOrNot = await this._prismaCli.advisor_AcademicWork.findFirst({
        where: {
          advisorId,
          academicWorkId,
        },
      });
      if (entityOrNot === null) return new Error("Not found table with advisorId e AcademicWorkId");

      if (entityOrNot.isMain) {
        await this._prismaCli.academicWork.update({
          where: { id: entityOrNot.academicWorkId },
          data: { academicWorkVisibility: false },
        });
      }
  ...
  async updateAcademicWork(project: updateAcademicWorkDTO, id: string): Promise<Error | IReturnAcademicWorkDTO> {
    ...
    try {
      ...
      operations.push(
        this._prismaCli.academicWork.update({
          where: {
            id: id,
          },
          data: cleanedData,
  ...
  private mapperAcademicWork(prismaData: any): Error | AcademicWork {
    ...
    const advisorMapped: Advisor[] = prismaData.advisor.map((advisor: any) => {
      ...
      else return advisorOrError;
    });

    if (advisorErrorsMapping.length > 0) return Error("ERROR_TO_MAPPING_ADVISOR", { cause: advisorErrorsMapping });

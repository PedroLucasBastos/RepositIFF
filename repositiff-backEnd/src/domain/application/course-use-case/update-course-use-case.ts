...
export class UpdateCourseUseCase {
    ...
    async execute(props: ICourseUpdateProps): Promise<Either<DomainError, Course>> {
        ...
        if (parametersValidToUpdateOrNot.isLeft())
            return new Left(parametersValidToUpdateOrNot.value)
        const resultOrError = await this._repo.updateCourse(courseId, updateFields);
        if (resultOrError instanceof Error)
            return new Left(
                new DomainError(
                    ErrorCategory.Application,
                    "ERROR_TO_UPDATE_COURSE",
    ...
    updateEntityProps(props: ICourseUpdateFields): Either<DomainError, void> {
        ...
        const resultList = []
        if (name)
            resultList.push(CourseValidator.validateNameField(name));
        if (courseCode)
            resultList.push(CourseValidator.validateCourseCodeField(courseCode));
        if (degreeType)
            resultList.push(CourseValidator.validateDegreeType(degreeType));

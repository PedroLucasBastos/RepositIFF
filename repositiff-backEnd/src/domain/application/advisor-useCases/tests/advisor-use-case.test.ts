...
describe("Test the guiding actor use cases", () => {
  ...
  it("Should ble able to create and registred a dvisor in database", async () => {
    ...
    const advisorExisting = await repo.findAdvisorById(advisor.id);
  ...
  it("Should not be able to register a new advisor with a registration number that is already in use", async () => {
    const useCase = new CreateAdvisorUseCase(repo);
    const advisorRegistratonNumberInvalid =
      AdvisorsProducts.duplicateRegistrationNumber();
    const advisorUseCaseOrError = await useCase.execute(
      advisorRegistratonNumberInvalid
    );
    ...
    expect(errorDomain.details).toEqual(
      "Duplicate value for unique field(s) - PRISMA ERROR CODE P2002"
    );
  });
  ...
  it("It should be able to update the surname of a supervisor registered in the database", async () => {
    ...
    const { registrationNumber, surname } =
      AdvisorsProducts.updateSurnameValid();

    const advisorOrError =
      await repo.findAdvisorByRegistrationNumber(registrationNumber);
  ...
  it("Should be able to delete a advisor registred in databse with id", async () => {
    ...
    const verification = await repo.findAdvisorByRegistrationNumber(
      advisorToDelete.registrationNumber
    );
